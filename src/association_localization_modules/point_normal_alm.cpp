...
void PointNormalALM::configure(tue::Configuration config)
{
    if (config.readGroup("parameters"))
    {
        ...
        config.value("curvature_weight", curvature_weight_);
    ...
    float cw = curvature_weight_;
    float alpha[4] = {pw,pw,pw,cw};
                             ...
                             std::map<UUID, EntityConstPtr>& entities)
{
    ...
    std::vector<const Entity*> world_model_pc_entity_ptrs;
    {
        ...
        world_model_npcl =  helpers::ddp::pclToNpcl(world_model_pcl_downsampled, 15);
        profiler_.stopTimer();
    }
    ...
    std::vector<const ed::Entity*> associations(sensor_data.point_cloud_with_normals->size());
    {
        ...
        for (unsigned int i = 0; i < sensor_data.point_cloud_with_normals->size(); ++i) {

            // Find corresponding indices and sqr distances
            std::vector<int> k_indices;
            std::vector<float> k_sqr_distances;
            tree_->nearestKSearch(*sensor_data.point_cloud_with_normals, i, 1, k_indices, k_sqr_distances);

            // Association criteria
            bool associated = k_sqr_distances[0] < association_correspondence_distance_ * association_correspondence_distance_;
...
//    cv::imshow("rendered", wm_depth_img / 8);
//    cv::waitKey();
