...
GUIServer::GUIServer() : map_draw_thread_busy_(false)
{
    command_ = "explore";
    ...
    if (srv_get_measurements_.getService() != srv_get_measurements)
    {
        ros::AdvertiseServiceOptions opt_get_measurements =
                ros::AdvertiseServiceOptions::create<ed::GetMeasurements>(
                    srv_get_measurements, boost::bind(&GUIServer::srvGetMeasurements, this, _1, _2), ros::VoidPtr(), &cb_queue_);
    ...
    if (srv_set_label_.getService() != srv_set_label)
    {
        ros::AdvertiseServiceOptions opt_set_label =
                ros::AdvertiseServiceOptions::create<ed::SetLabel>(
                    srv_set_label, boost::bind(&GUIServer::srvSetLabel, this, _1, _2), ros::VoidPtr(), &cb_queue_);
    ...
    if (srv_raise_event_.getService() != srv_raise_event)
    {
        ros::AdvertiseServiceOptions opt_raise_event =
                ros::AdvertiseServiceOptions::create<ed::RaiseEvent>(
                    srv_raise_event, boost::bind(&GUIServer::srvRaiseEvent, this, _1, _2), ros::VoidPtr(), &cb_queue_);
    ...
    if (srv_get_command_.getService() != srv_get_command)
    {
        ros::AdvertiseServiceOptions opt_get_command =
                ros::AdvertiseServiceOptions::create<ed::GetGUICommand>(
                    srv_get_command, boost::bind(&GUIServer::srvGetCommand, this, _1, _2), ros::VoidPtr(), &cb_queue_);
...
cv::Point2i GUIServer::coordinateToPixel(const geo::Vector3& p) const
{
...
ed::UUID GUIServer::getEntityFromClick(const cv::Point2i& p) const
{
    for(std::map<ed::UUID, EntityData>::const_iterator it_entity = wm_entities_copy_.begin(); it_entity != wm_entities_copy_.end(); ++it_entity)
    {
        const EntityData& e = it_entity->second;
        if (!e.shape)
        {
            const pcl::PointCloud<pcl::PointXYZ>& chull_points = e.convex_hull.chull;
...
void GUIServer::publishMapImage()
{
    map_draw_thread_busy_ = true;
    ...
    for(std::map<ed::UUID, EntityData>::const_iterator it_entity = wm_entities_copy_.begin(); it_entity != wm_entities_copy_.end(); ++it_entity)
    {
        ...
        if (!chull_points.empty())
        {
            ...
            for(unsigned int i = 0; i < chull_points.size(); ++i)
            {
                ...
                cv::line(map_image_,
                         coordinateToPixel(p1.x, p1.y, e.convex_hull.min_z),
                         coordinateToPixel(p2.x, p2.y, e.convex_hull.min_z),
                         0.3 * color, thickness);
            }
            ...
            for(unsigned int i = 0; i < chull_points.size(); ++i)
            {
                ...
                cv::line(map_image_,
                         coordinateToPixel(p.x, p.y, e.convex_hull.min_z),
                         coordinateToPixel(p.x, p.y, e.convex_hull.max_z),
                         0.5 * color, thickness);
            }
            ...
            for(unsigned int i = 0; i < chull_points.size(); ++i)
            {
                ...
                cv::line(map_image_,
                         coordinateToPixel(p1.x, p1.y, e.convex_hull.max_z),
                         coordinateToPixel(p2.x, p2.y, e.convex_hull.max_z),
                         color, thickness);
            }
            ...
            if (e.type == "person")
            {
                ...
                cv::circle(map_image_, coordinateToPixel(e.convex_hull.center_point), 15, cv::Scalar(255, 0, 0), 10);
            }
        }
    }
    ...
    {
        const EntityData& e = it_entity->second;
        if (e.last_measurement && (!last_measurement || e.last_measurement->timestamp() > last_measurement->timestamp()))
            last_measurement = e.last_measurement;
    }
...
//    cv::waitKey(3);
    ...
    map_draw_thread_busy_ = false;
}

// ----------------------------------------------------------------------------------------------------

void GUIServer::update(std::map<ed::UUID, ed::EntityConstPtr>& entities)
{
    wm_entities_ = &entities;

    handleRequests();

    if (!map_draw_thread_busy_ && trigger_map_publish_.triggers())
    {
        wm_entities_copy_.clear();
        ...
        for(std::map<ed::UUID, ed::EntityConstPtr>::const_iterator it = entities.begin(); it != entities.end(); ++it)
        {
            ...
            // Copy data for threaded processing
            EntityData data;
            data.id = e->id();
            data.type = e->type();
            data.convex_hull = e->convexHull();
            data.shape = e->shape();
            data.last_measurement = e->lastMeasurement();
            data.pose = e->pose();

            wm_entities_copy_[it->first] = data;
        }

        map_publish_thread_ = boost::thread(boost::bind(&GUIServer::publishMapImage, this));
    }
...
bool GUIServer::srvGetMeasurements(ed::GetMeasurements::Request& req, ed::GetMeasurements::Response& res)
{   
    if (selected_id_.empty())
        return true;

    std::map<ed::UUID, ed::EntityConstPtr>::const_iterator it = wm_entities_->find(selected_id_);
    if (it == wm_entities_->end())
        return true;
...
bool GUIServer::srvSetLabel(ed::SetLabel::Request& req, ed::SetLabel::Response& res)
{
    ed::UUID id = req.id;
    if (id.empty())
        id = selected_id_;
    ...
    std::map<ed::UUID, ed::EntityConstPtr>::iterator it = wm_entities_->find(id);
    if (it == wm_entities_->end())
    {
    ...
    else
    {
        ...
        // set the updated entity in the map
        it->second = e_updated;
...
bool GUIServer::srvRaiseEvent(ed::RaiseEvent::Request& req, ed::RaiseEvent::Response& res)
{
    ...
    if (req.name == "click")
    {
        ...
        if (parseParam(params, "x", x) && parseParam(params, "y", y) && parseParam(params, "type", type))
        {
            ...
            if (type == "delete")
            {
                if (!selected_id_.empty())
                {
                    wm_entities_->erase(selected_id_);
...
bool GUIServer::srvGetCommand(ed::GetGUICommand::Request& req, ed::GetGUICommand::Response& res)
{
    if (!command_.empty())
    {
