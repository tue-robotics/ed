...
public:
    ...
    {
        auto const it = volumes_added.find(id);
        if (it != volumes_added.end())
        {
            auto const it2 = it->second.find(Volume_name);
    ...
    {
        volumes_removed[id].insert(Volume_name);
        flagUpdated(id);
    }
    ...
    void setConvexHullNew(const UUID& id,
    ...
    {
        type_sets_added[id].insert(type);
        flagUpdated(id);
    }
    ...
    {
        type_sets_removed[id].insert(type);
        flagUpdated(id);
    }
    ...
    template <typename T> void setProperty(const UUID& id, const PropertyKey<T>& key, const T& value)
    {
        if (!key.valid())
            return;
