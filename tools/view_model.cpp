...
static geo::DepthCamera cam;

static geo::Vector3 cam_lookat;
static double cam_dist, cam_yaw, cam_pitch;
static cv::Point LAST_MOUSE_POS;
static bool do_rotate = true;
static geo::Pose3D cam_pose;

static bool do_flyto = false;
static geo::Vector3 cam_lookat_flyto;

static bool render_required = true;
...
static void CallBackFunc(int event, int x, int y, int flags, void* /*userdata*/)
{
    ...
    {
        double const dx = x - LAST_MOUSE_POS.x;
        double const dy = y - LAST_MOUSE_POS.y;
...
int main(int argc, char** argv)
{
    ...
    // Set camera specs
    cam = geo::DepthCamera(CANVAS_WIDTH,
                           CANVAS_HEIGHT,
                           0.87 * CANVAS_WIDTH,
                           0.87 * CANVAS_WIDTH,
                           (CANVAS_WIDTH / 2) + 0.5,
    ...
    cv::setMouseCallback("visualization", CallBackFunc, nullptr);
    ...
    while (true)
    {
        ...
        geo::Vector3 const rz = -(cam_lookat - cam_pose.t).normalized();
        ...
        if (render_required)
        {
            ...
            image =
                cv::Mat(depth_image.rows, depth_image.cols, CV_8UC3, cv::Scalar(20, 20, 20)); // Not completely black
            ed::renderWorldModel(world_model, show_volumes, cam, cam_pose.inverse(), depth_image, image);
            render_required = false;
        }

        cv::imshow("visualization", image);
        char const key = cv::waitKey(10);
        ...
        if (do_flyto)
        {
            geo::Vector3 const diff = cam_lookat_flyto - cam_lookat;
            double const dist = diff.length();

            double const max_dist = std::max(0.001 * cam_dist, dist * 0.1);
            if (dist < max_dist)
            {
