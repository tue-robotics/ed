...
public:
   ...
   */
  std::map<std::string, float> process_image(const cv::Mat& camera_image) const;
...
protected:
  ...
  */
    void update_matches_map(const vt::Matches& matches, size_t size, std::map<uint32_t, float>& matches_map) const;
   ...
   */
    Keypoint extract_keypoints(const cv::Mat& image, bool frames_only = false) const;
};
}
